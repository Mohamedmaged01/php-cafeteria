...
class PHPMailer
{
    ...
    /**
     * The SMTP HELO/EHLO name used for the SMTP myconnection.
    ...
    /**
     * What kind of encryption to use on the SMTP myconnection.
     ...
     * @see SMTP::DEBUG_myconnection: As SERVER plus myconnection status
    ...
    /**
     * Whether to keep the SMTP myconnection open after each message.
     * If this is set to true then the myconnection will remain open after a send,
     * and closing the myconnection will require an explicit call to smtpClose().
    ...
    {
        //Close any open SMTP myconnection nicely
        $this->smtpClose();
    }
    ...
    /**
     * Initiate a myconnection to an SMTP server.
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ($this->smtp->connect($prefix . $host, $port, $this->Timeout, $options)) {
                try {
                    ...
                    $lastexception = $exc;
                    $this->edebug($exc->getMessage());
                    //We must have connected, but then failed TLS or Auth, so close myconnection nicely
                    $this->smtp->quit();
                }
            }
        }
        //If we get here, all myconnection attempts have failed, so close myconnection hard
        $this->smtp->close();
    ...
    protected function lang($key)
    {
        ...
        if (array_key_exists($key, $this->language)) {
            if ('smtp_connect_failed' === $key) {
                //Include a link to troubleshooting docs on SMTP myconnection failure.
